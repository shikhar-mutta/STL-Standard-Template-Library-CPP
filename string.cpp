...
int main() {
    ...
    cout << "empty():   " << s.empty()    << "\n";
    s.reserve(100); cout << "after reserve(100): capacity=" << s.capacity() << "\n";
    s.shrink_to_fit();
    s.resize(5);    cout << "after resize(5): " << s << "\n";
    ...
    str[0] = 'A'; cout << "after str[0]='A': " << str << "\n";

    // 4. Modifiers
    cout << "\n--- 4. Modifiers ---\n";
    string m = "hello";
    m += " world";            cout << "+=:             " << m << "\n";
    m.append("!!!");          cout << "append():       " << m << "\n";
    m.append(3, '?');         cout << "append(3,'?'):  " << m << "\n";
    m.push_back('!');         cout << "push_back:      " << m << "\n";
    m.pop_back();             cout << "pop_back:       " << m << "\n";
    m.insert(5, " C++20");    cout << "insert(5,...):  " << m << "\n";
    m.erase(0, 6);            cout << "erase(0,6):     " << m << "\n";
    m.replace(0, 5, "Hi");    cout << "replace(0,5,...): " << m << "\n";
    m.assign("brand new string"); cout << "assign():     " << m << "\n";
    ...
    ranges::sort(alg_s);    cout << "sorted:   " << alg_s << "\n";
    alg_s = "programming";
    ranges::reverse(alg_s); cout << "reversed: " << alg_s << "\n";
    alg_s = "programming";
    ...
    string token;
    cout << "tokens: ";
    while (ss >> token) cout << token << " "; cout << "\n";

    string csv = "alice,bob,charlie";
    stringstream csv_ss(csv);
    cout << "CSV: ";
    while (getline(csv_ss, token, ',')) cout << token << " "; cout << "\n";

    string nums_str = "10 20 30 40 50";
    stringstream num_ss(nums_str);
    vector<int> nums; int nn;
    while (num_ss >> nn) nums.push_back(nn);
    ...
    for (int i=0, j=0; i<(int)txt.size(); ) {
        if (txt[i]==pat[j]) { i++; j++; }
        if (j==(int)pat.size()) { cout << i-j << " "; j=lps[j-1]; }
        else if (i<(int)txt.size() && txt[i]!=pat[j]) { if (j) j=lps[j-1]; else i++; }
    }
    cout << "\n";
