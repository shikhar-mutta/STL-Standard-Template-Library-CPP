...
int main() {
    ...
    for (int x : {3,1,4,1,5,9,2}) maxpq.push(x);
    ...
    pair_pq.push({3,'a'}); pair_pq.push({1,'c'}); pair_pq.push({2,'b'});
    cout << "pair min-heap (priority:char): ";
    while (!pair_pq.empty()) {
        auto [pri,val] = pair_pq.top(); pair_pq.pop();
        cout << pri << ":" << val << " ";
    }
    cout << "\n";
    ...
    for (int x : data) { k_large.push(x); if ((int)k_large.size() > k) k_large.pop(); }
    ...
    for (int x : data) { k_small.push(x); if ((int)k_small.size() > k) k_small.pop(); }
    ...
    vector<int> dist(nn, INF); dist[0] = 0;
    priority_queue<pair<int,int>,vector<pair<int,int>>,greater<>> dijkstra_pq;
    dijkstra_pq.push({0,0});
    while (!dijkstra_pq.empty()) {
        auto [d,u] = dijkstra_pq.top(); dijkstra_pq.pop();
        if (d > dist[u]) continue;
        for (auto [vv,w] : adj[u])
            if (dist[u]+w < dist[vv]) { dist[vv]=dist[u]+w; dijkstra_pq.push({dist[vv],vv}); }
    }
    cout << "Dijkstra from 0: ";
    for (int i = 0; i < nn; i++) cout << i << ":" << dist[i] << " "; cout << "\n";
    ...
    for (int i = 0; i < (int)arrays.size(); i++) merge_pq.push({arrays[i][0],i,0});
    cout << "merged: ";
    while (!merge_pq.empty()) {
        auto [val,ai,ei] = merge_pq.top(); merge_pq.pop();
        cout << val << " ";
        if (ei+1 < (int)arrays[ai].size()) merge_pq.push({arrays[ai][ei+1],ai,ei+1});
    }
    cout << "\n";
    ...
    while (!task_pq.empty()) {
        ...
        while (cycle-- && !task_pq.empty()) {
            temp.push_back(task_pq.top()-1); task_pq.pop(); time_units++;
        }
        for (int t : temp) if (t > 0) task_pq.push(t);
        if (!task_pq.empty()) time_units += cycle+1;
    }
    ...
    cout << "\n--- 9. From Ranges ---\n";
    auto source = views::iota(1,11) | views::transform([](int x){ return x*x; });
