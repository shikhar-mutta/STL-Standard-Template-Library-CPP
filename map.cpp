...
int main() {
    ...
    em.erase("b"); print_map("after erase('b')", em);
    em.erase(em.find("c")); print_map("after erase by iter", em);
    em.erase(em.begin(), em.find("d")); print_map("after range erase", em);
    em.clear(); cout << "after clear, size: " << em.size() << "\n";

    // 5. Iterators
    cout << "\n--- 5. Iterators ---\n";
    map<int,string> im = {{1,"one"},{2,"two"},{3,"three"}};
    cout << "forward: "; for (auto& [k,v] : im) cout << k << ":" << v << " "; cout << "\n";
    cout << "reverse: "; for (auto rit = im.rbegin(); rit != im.rend(); ++rit) cout << rit->first << ":" << rit->second << " "; cout << "\n";
    ...
    auto high = scores | views::filter([](auto& p){ return p.second >= 90; });
    cout << "scores >= 90: "; for (auto& [k,v] : high) cout << k << ":" << v << " "; cout << "\n";
    vector<string> keys; for (auto& [k,v] : scores) keys.push_back(k);
    cout << "keys: "; for (auto& k : keys) cout << k << " "; cout << "\n";
    vector<int> vals; for (auto& [k,v] : scores) vals.push_back(v);
    cout << "vals: "; for (int v : vals) cout << v << " "; cout << "\n";
    auto max_it = ranges::max_element(scores, [](auto& a, auto& b){ return a.second < b.second; });
    ...
    map<int,int> cnt; for (int x : data_arr) cnt[x]++;
    cout << "distinct elements: " << cnt.size() << "\n";
    auto max_elem = *ranges::max_element(cnt, [](auto& a, auto& b){ return a.second < b.second; });
