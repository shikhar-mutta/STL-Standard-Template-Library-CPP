...
int main()
{
    ...
    ls.sort();
    print_list("sorted", ls);
    ls.unique();
    print_list("unique", ls);
    ls.reverse();
    print_list("reversed", ls);
    ls.remove(5);
    print_list("remove(5)", ls);
    ls.remove_if([](int x)
                 { return x < 3; });
    print_list("remove_if(<3)", ls);
    ...
    // ***
    m1.merge(m2);
    print_list("merged", m1); // 1 2 3 4 5 6 7 8
    ...
    cout << " \n";
    cout << "forward: ";
    for (auto rit = il.begin(); rit != il.end(); ++rit)
    {
        cout << *rit << " ";
    }
    cout << " \n";
    cout << "reverse: ";
    for (auto rit = il.rbegin(); rit != il.rend(); ++rit)
    {
        cout << *rit << " ";
    }
    cout << " \n";
    ...
    // filter
    auto big = rl | views::filter([](int x)
                                  { return x > 2; });
    cout << "elements > 2:  ";
    for (int x : big)
    {
    ...
    cout << " \n";
    // map
    auto sq = rl | views::transform([](int x)
                                    { return x * x; });
    cout << "squared:       ";
    for (int x : sq)
    {
    ...
    // ***
    advance(fit2, 2); // Moves iterator 2 positions forward
    fl2.insert_after(fit2, 3);
    print_fl("after insert_after", fl2);
    advance(fit2, 1); // Moves iterator 1 position forward
    ...
    print_fl("unique", fa);
    fa.reverse();
    print_fl("reversed", fa);
    fa.remove_if([](int x)
                 { return x > 4; });
    print_fl("remove_if(>4)", fa);
