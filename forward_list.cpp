...
int main() {
    ...
    fl.push_front(1); fl.emplace_front(0);
    ...
    lal.sort();    print_fl("sorted", lal);
    lal.unique();  print_fl("unique", lal);
    lal.reverse(); print_fl("reversed", lal);
    lal.remove(6); print_fl("remove(6)", lal);
    lal.remove_if([](int x){ return x < 3; }); print_fl("remove_if(<3)", lal);
    ...
    auto evens = rl | views::filter([](int x){ return x%2==0; });
    cout << "evens:   "; for (int x : evens) cout << x << " "; cout << "\n";
    auto doubled = rl | views::transform([](int x){ return x*2; });
    cout << "doubled: "; for (int x : doubled) cout << x << " "; cout << "\n";
    ...
    for (int x : part) {
        if (x%2==0) evens_list.push_front(x);
        else odds_list.push_front(x);
    }
    evens_list.sort(); odds_list.sort();
