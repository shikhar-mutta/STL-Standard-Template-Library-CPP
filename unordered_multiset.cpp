...
void demo_unordered_multiset() {
    ...
    cout << "ums (with dups, unordered): "; for (int x : ums) cout << x << " "; cout << "\n";
    cout << "size: " << ums.size() << "  count(5): " << ums.count(5) << "\n";

    ums.insert(10); ums.insert(10); ums.emplace(7);
    ...
    cout << "equal_range(5): ";
    for (auto it = er.first; it != er.second; ++it) cout << *it << " ";
    ...
    if (oneit != ums.end()) ums.erase(oneit);
    cout << "after erase one 5: count(5)= " << ums.count(5) << "\n";

    // Merge
    unordered_multiset<int> A = {1,2,2}, B = {2,3,3};
    A.merge(B);
    cout << "A after merge: "; for (int x : A) cout << x << " "; cout << "\n";
    cout << "B after merge (empty): "; for (int x : B) cout << x << " "; cout << "\n";

    // Ranges
    auto evens = ums | views::filter([](int x){ return x % 2 == 0; });
    vector<int> evens_vec(evens.begin(), evens.end());
    ranges::sort(evens_vec);
    cout << "evens (sorted): "; for (int x : evens_vec) cout << x << " "; cout << "\n";
...
void demo_unordered_multimap() {
    ...
    umm.insert({"banana",7}); umm.emplace("date",3);
    ...
    cout << "apple values: "; for (auto it=lo; it!=hi; ++it) cout << it->second << " "; cout << "\n";
    ...
    for (auto& [lang, _] : index) {
        if (printed.contains(lang)) continue;
        printed.insert(lang);
        cout << "  " << lang << ":";
    ...
    Ma.emplace(1,10); Ma.emplace(2,20);
    Mb.emplace(2,99); Mb.emplace(3,30);
    Ma.merge(Mb);
    cout << "Ma after merge: "; for (auto& [k,v] : Ma) cout << k << ":" << v << " "; cout << "\n";
    cout << "Mb (empty): "; for (auto& [k,v] : Mb) cout << k << ":" << v << " "; cout << "\n";

    // Ranges
    auto filtered = index | views::filter([](auto& p){ return p.first == "cpp"; });
    cout << "cpp entries: "; for (auto& [k,v] : filtered) cout << v << " "; cout << "\n";
}
