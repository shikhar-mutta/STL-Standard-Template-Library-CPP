...
void print_set(const string& label, const set<int>& s) {
    cout << label << ": "; for (int x : s) cout << x << " "; cout << "\n";
}
...
int main() {
    ...
    s.insert({20,30,40}); s.emplace(15); s.emplace(10);
    ...
    es.erase(3);                          print_set("after erase(3)", es);
    es.erase(es.find(2));                 print_set("after erase(find(2))", es);
    es.erase(es.lower_bound(4), es.end()); print_set("after erase [lb(4),end)", es);
    es.clear(); cout << "after clear, size: " << es.size() << "\n";

    // 6. Iterators
    cout << "\n--- 6. Iterators ---\n";
    set<int> is = {10,20,30,40};
    cout << "forward: "; for (int x : is) cout << x << " "; cout << "\n";
    cout << "reverse: "; for (auto rit = is.rbegin(); rit != is.rend(); ++rit) cout << *rit << " "; cout << "\n";
    ...
    auto evens = rng_s | views::filter([](int x){ return x%2==0; });
    cout << "evens:      "; for (int x : evens) cout << x << " "; cout << "\n";
    ...
    cout << "sorted by length: "; for (auto& w : word_set) cout << w << " "; cout << "\n";
