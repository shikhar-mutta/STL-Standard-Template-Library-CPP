...
void print_deque(const string& label, const deque<T>& d) {
    cout << label << ": "; for (auto& x : d) cout << x << " "; cout << "\n";
}
...
int main() {
    ...
    cout << "empty():    " << d1.empty()    << "\n";
    d2.resize(7, 99); print_deque("after resize(7,99)", d2);
    ...
    d = {2,3,4};
    d.push_front(1); d.push_back(5);
    d.emplace_front(0); d.emplace_back(6);
    ...
    cout << "\n--- 5. Insert & Erase ---\n";
    d.insert(d.begin()+1, 99); print_deque("after insert(1,99)", d);
    d.insert(d.end(), {100,200}); print_deque("after insert range", d);
    d.erase(d.begin()); print_deque("after erase(begin)", d);
    d.erase(d.begin(), d.begin()+2); print_deque("after erase [0,2)", d);
    d.clear(); cout << "after clear, size: " << d.size() << "\n";

    // 6. Algorithms
    cout << "\n--- 6. Algorithms ---\n";
    deque<int> ad = {5,3,1,4,2};
    ranges::sort(ad); print_deque("sorted", ad);
    ranges::reverse(ad); print_deque("reversed", ad);
    ranges::sort(ad);
    ...
    for (int x : {1,2,3,4,5,6,7}) {
        circ.push_back(x);
        if ((int)circ.size() > capacity) circ.pop_front();
        cout << "add " << x << " -> ["; for (int v : circ) cout << v << " "; cout << "]\n";
    }
