...
void print_ms(const string& label, const multiset<int>& ms) {
    cout << label << ": "; for (int x : ms) cout << x << " "; cout << "\n";
}
...
int main() {
    ...
    cout << "ms_desc: "; for (int x : ms_desc) cout << x << " "; cout << "\n";

    // 2. Insert
    cout << "\n--- 2. Insert ---\n";
    multiset<int> s;
    s.insert(10); s.insert(20); s.insert(10);
    s.insert({5,25,20}); s.emplace(15);
    ...
    es.erase(3); print_ms("after erase(3) all", es);
    auto one = es.find(2); es.erase(one);
    ...
    int k = 4;
    auto kit = kth.begin(); advance(kit, k-1);
    cout << k << "-th smallest: " << *kit << "\n";

    // 8. Ranges
    cout << "\n--- 8. Ranges ---\n";
    multiset<int> rs = {1,2,2,3,3,3,4,5};
    auto freq2 = rs | views::filter([](int x){ return x >= 3; });
    cout << "elements >= 3: "; for (int x : freq2) cout << x << " "; cout << "\n";
